...
imagecounter = 0
imagefolder = 'Photos'
templatePath = os.path.join('Photos', 'Template', "template2.png") #Path of template image
ImageShowed = False
Printing = False
BUTTON_PIN = 21
#IMAGE_WIDTH = 558
...
#            background.fill(pygame.Color("black"))
            background.fill(camera.start_preview(resolution=(1920,1080) ) )
    if (textToDisplay != ""):
            #print(displaytext)
#            font = pygame.font.Font(None, fontSize)
            font = pygame.font.Font("./Comfortaa-Bold.ttf", fontSize)
            text = font.render(textToDisplay, 1, (35, 0, 39))
...
def UpdateDisplay():
    ...
    if (Message != ""):
            #print(displaytext)
            font = pygame.font.Font("./Comfortaa-Bold.ttf", 125)
            text = font.render(Message, 1, (35, 0, 93))
            textpos = text.get_rect()
            textpos.centerx = background.get_rect().centerx
            textpos.centery = background.get_rect().centery
            if(ImageShowed):
    ...
    if (Numeral != ""):
            #print(displaytext)
            font = pygame.font.Font("./Comfortaa-Bold.ttf", 800)
            text = font.render(Numeral, 1, (35, 0, 93))
            textpos = text.get_rect()
            textpos.centerx = background.get_rect().centerx
            textpos.centery = background.get_rect().centery
            if(ImageShowed):
    ...
    if (CountDownPhoto != ""):
            #print(displaytext)
            font = pygame.font.Font("./Comfortaa-Bold.ttf", 500)
            text = font.render(CountDownPhoto, 1, (35, 0, 93))
            textpos = text.get_rect()
            textpos.centerx = background.get_rect().centerx
            textpos.centery = background.get_rect().centery
            if(ImageShowed):
                ...
                if (TotalImageCount <= PhotosPerCart):
                        if os.path.isfile('/home/pi/Desktop/tempprint.jpg'):
                                ...
                                if printqueuelength > 1:
                                        ...
                                        Message = "Nao foi possivel imprimir"
                                        UpdateDisplay()
                                        time.sleep(1)
                                else:
...
def main(threadName, *args):
    InitFolder()
    while True:
        show_image('assets/start_camera2.png')
        WaitForEvent()
        time.sleep(0.1)
        TakePictures()
    GPIO.cleanup()
